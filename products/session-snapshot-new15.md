...
# Session Snapshot - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Snapshot the whole window set so monday-you resumes in one click: the tabs, the layout, and the re-entry note. The product is a local-first Chrome extension: snapshot records with 9 domain fields, a 4-gate quality workflow, readiness scoring, milestones, and exportable artifacts. Free plan: 5 active snapshots; Pro (Freemius-licensed, not yet configured) lifts the limit.

## What it replaces

- [paoloronco/tab-session-saver](https://github.com/paoloronco/tab-session-saver)
- [SnapzBack Layout Manager - Windows & Tabs – Get this Extension for 🦊 Firefox (en-US)](https://addons.mozilla.org/en-US/firefox/addon/snapzback/)
- [Tabwell — Save and restore browser sessions, with tab groups](https://tabwell.org/)
- [Sessionat for Chrome: Tab & Session Manager with Cloud Sync | Sessionat](https://sessionat.com/extension)

## The wedge (why ours wins)

re-entry note is actionable: this build proves it in code (8 readiness points, suite-verified); topic grouping for big sets: this build proves it in code (6 readiness points, suite-verified); clean restore verified: this build proves it in code (6 readiness points, suite-verified); Local-first privacy boundary: only storage, tabs permission; competitors route through their cloud; Zero signup: works on install; the competitors above gate basic use behind accounts

## What it does today

- Engine: 9 typed domain fields with validators, 4-gate quality workflow, weighted readiness scoring with 3 product-specific bonus rules, 3 milestones, 3 artifact generators, duplicate detection, search, free-limit enforcement, bounded import/export.
- Automated suite: 48 checks, all green.
- Canonical 297-map entry: feature #256 (Studio plan).
...
1. Click the **Session Snapshot** icon in the Chrome toolbar (top right, puzzle-piece menu if pinned away).
...
Session Snapshot today carries 1 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
