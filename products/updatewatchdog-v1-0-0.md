...
# Update Watchdog - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Update manager. The product is a WordPress plugin with a pure-PHP testable engine, a folk-styled admin report, and a bounded, evidence-first workflow. Fully local: nothing leaves the server.

## What it replaces

- [wordpress.org](https://wordpress.org/plugins/update-compass/)
- [UpdateGuard – Safe Update Manager — WordPress Plugin - XeroWP](https://xerowp.com/plugins/updateguard)

## The wedge (why ours wins)

Changelog digest for exactly the versions being skipped - competitors show the latest notes or none; Risk flags (security/major/breaking/year-quiet) computed, not eyeballed; One-click rollback point before updating - competitors stop at notification; Fully local privacy boundary: no external service, no account, no data leaving the server; Zero signup: works on activation; competitors gate basics behind accounts or upsells
...
2. In the left sidebar, click **Updatewatchdog**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
...
Update Watchdog today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
