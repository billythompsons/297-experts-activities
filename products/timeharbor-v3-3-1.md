...
# TimeHarbor - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Capacity planning: members with weekly hour ceilings, project allocations that can never exceed capacity, time entries gated on allocations, utilization reviews per week, and recorded workload handoffs that respect the target's remaining capacity.

_Freemius store product #38543._

## What it replaces

- [Timesheet](https://wordpress.org/plugins/timesheet/)
- [Employee Roster](https://wordpress.org/plugins/employee-roster/)
- [PlainTracker](https://wordpress.org/plugins/plaintracker/)
- [Dynamic Time](https://wordpress.org/plugins/dynamic-time/)

## The wedge (why ours wins)

Capacity is enforced at allocation time: a week can never be booked past a member's stated hours - the engine refuses; Time entries only exist against real allocations, so the plan/actual review (utilization %) is always computable per member per week; Handoffs are real ledger moves: minutes leave the source, land on the target (who must have room), and the transfer is permanently recorded
...
2. In the left sidebar, click **TimeHarbor**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
3. The top of the page carries the live summary - read it left to right for the state of things.
4. The main sections are **People & projects, Allocate / log / hand off, Handoff record** - work top to bottom.
...
TimeHarbor today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
