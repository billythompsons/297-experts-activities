...
# Tab Mirror - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Capture a window's tabs as a named session and mirror it to another window or device. The product is a local-first Chrome extension: session records with 8 domain fields, a 5-gate quality workflow, readiness scoring, milestones, and exportable artifacts. Free plan: 3 active sessions; Pro (Freemius-licensed, not yet configured) lifts the limit.

_Freemius store product #38854._
...
multi-tab session: this build proves it in code (10 readiness points, suite-verified); shared session: this build proves it in code (5 readiness points, suite-verified); high priority restore: this build proves it in code (5 readiness points, suite-verified); Local-first privacy boundary: only storage, tabs permission; competitors route through their cloud; Zero signup: works on install; the competitors above gate basic use behind accounts

## What it does today

- Engine: 8 typed domain fields with validators, 5-gate quality workflow, weighted readiness scoring with 3 product-specific bonus rules, 3 milestones, 3 artifact generators, duplicate detection, search, free-limit enforcement, bounded import/export.
- Automated suite: 40 checks, all green.
- Canonical 297-map entry: feature #279 (Studio plan).
...
1. Click the **Tab Mirror** icon in the Chrome toolbar (top right, puzzle-piece menu if pinned away).
...
Tab Mirror today carries 1 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
