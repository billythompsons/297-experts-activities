...
# Print Perfect - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Perfect the print before the ink: what gets stripped, what gets kept, the scale, and the proof page count. The product is a local-first Chrome extension: printjob records with 10 domain fields, a 4-gate quality workflow, readiness scoring, milestones, and exportable artifacts. Free plan: 8 active print jobs; Pro (Freemius-licensed, not yet configured) lifts the limit.

## What it replaces

- [Print - Chrome Web Store](https://chromewebstore.google.com/detail/print/ffcbjhilgfookfikcfhlidlmedmnchde)
- [PrintFriendly — Edit PDFs and Print Web Pages Without Ads ...](https://www.printfriendly.com/)
- [Smart Content Extraction — How We Remove the Clutter — Pretty PDF Printer](https://prettypdfprinter.com/resources/features/smart-content-extraction)
- [PDF Preflight Checker | PDF Press](https://pdfpress.app/pdf-preflight-checker)

## The wedge (why ours wins)

ink saver engaged: this build proves it in code (8 readiness points, suite-verified); aggressive de-clutter: this build proves it in code (6 readiness points, suite-verified); test page discipline: this build proves it in code (6 readiness points, suite-verified); Local-first privacy boundary: only storage, activeTab, scripting permission; competitors route through their cloud; Zero signup: works on install; the competitors above gate basic use behind accounts

## What it does today

- Engine: 10 typed domain fields with validators, 4-gate quality workflow, weighted readiness scoring with 3 product-specific bonus rules, 3 milestones, 3 artifact generators, duplicate detection, search, free-limit enforcement, bounded import/export.
- Automated suite: 51 checks, all green.
- Canonical 297-map entry: feature #109 (Accelerator plan).

How the pieces fit:

```mermaid
flowchart LR
    A[Toolbar popup] --> B[Records view]
    A --> C[New entry form]
    B --> D[(Local stores)]
    C --> D
    D --> E[Artifacts + export]
    F[Tier gate] -. lifts limits .-> C
    G[Alarms] --> H[Reminders + streaks]
    D --> H
```
...
1. Click the **Print Perfect** icon in the Chrome toolbar (top right, puzzle-piece menu if pinned away).
...
Print Perfect today carries 1 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
...
- **Files:** `popup/modules/voicegauge.js` (engine), `popup/views/voicegauge-records.js` and `popup/views/voicegauge-new.js` (views), reusing `lib/storage.js`, `lib/gates.js`, `lib/export.js`; register the module in `popup/manifest.js`
- **Calls:** MediaRecorder API for capture; ffmpeg.wasm (`@ffmpeg/ffmpeg` from unpkg) for trim and format convert; Web Audio API for level metering; all local
- **Done when:** record -> trim -> export works offline and the transcript field saves with the clip
