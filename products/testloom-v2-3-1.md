...
# TestLoom - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Environment readiness engine: named environments, fixtures (table + row count) that must load in full before smoke checks can run, pass/fail results with history, computed readiness reports, and flakiness detection.

_Freemius store product #38540._

## What it replaces

- [WP Staging](https://wordpress.org/plugins/wp-staging/)
- [Upgrade Readiness Monitor](https://wordpress.org/plugins/upgrade-readiness-monitor/)
...
Ordering is enforced: smoke checks REFUSE to run until every fixture is loaded - no green reports from a half-seeded environment; Readiness is computed, not declared: all fixtures loaded, every check run, every latest result green. An unrun check blocks ready; Flakiness is detected from history: any check with both a pass and a fail is called out by name

## What it does today

- Automated suite: 22 passed, 0 failed.
...
2. In the left sidebar, click **TestLoom**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
3. The top of the page carries the live summary - read it left to right for the state of things.
4. The main sections are **New environment** - work top to bottom.
...
TestLoom today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
