...
# WorkflowWell - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Process mining for teams: staged workflows that measure per-stage dwell from real item transitions, computed bottlenecks, oldest-first WIP review, and improvement experiments that demand baseline AND result before a verdict.

_Freemius store product #38559._

## What it replaces

- [Fluent Boards](https://wordpress.org/plugins/fluent-boards/)
- [QuickTasker](https://wordpress.org/plugins/quicktasker/)
- [Pressio CRM](https://wordpress.org/plugins/pressio-crm/)
- [DoubleScale](https://wordpress.org/plugins/doublescale/)

## The wedge (why ours wins)

Dwell is measured per stage from real item transitions - the bottleneck is computed, not guessed, and "no data yet" is an honest answer; Improvements run as experiments: hypothesis, baseline, result, and a verdict that is impossible to record without BOTH measurements; WIP is always visible oldest-first, so aging items surface instead of hiding in a column

## What it does today

- Automated suite: 29 passed, 0 failed.
...
2. In the left sidebar, click **WorkflowWell**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
3. The top cards are the live numbers - **Workflows, In progress, Completed** - read them left to right for the state of things.
4. The main sections are **Experiments, New workflow, Track item, New experiment**. The records table's key columns: Stage, Avg dwell (min), Sitting now, ID, Hypothesis.
...
WorkflowWell today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
