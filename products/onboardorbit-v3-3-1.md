...
# Onboardorbit - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Customer and staff onboarding plans with dated milestones, evidence-gated completion, sign-off controls, and handoff packets a new owner can act on.

_Freemius store product #38485._

## The wedge (why ours wins)

Onboarding as an auditable process, not a tour: required milestones cannot complete without a 20+ character evidence note; Sign-off gate: plans cannot be signed off while required milestones are open; the gate is enforced in a pure-PHP engine, not just hidden in UI; Handoff packets: one call produces the completed-with-evidence and pending lists in due-date order - the artifact a new owner actually needs; Everything local: no SaaS connector, no external calls, state lives in a single option and exports cleanly
...
2. In the left sidebar, click **OnboardOrbit**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
3. The top of the page carries the live summary - read it left to right for the state of things.
4. The main sections are **New plan, Next actions**. The records table's key columns: Milestone, Due, Required, Status, Evidence.
...
Onboardorbit today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
