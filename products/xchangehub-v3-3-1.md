...
# XchangeHub - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Data exchange governance: typed field mappings, run-level validation that turns bad rows into reviewable exceptions, and completion gating that makes "the import finished" mean "every row is accounted for."

_Freemius store product #38560._
...
- [Import Export by RockstarLab](https://wordpress.org/plugins/import-export-by-rockstarlab/)

## The wedge (why ours wins)

Every row is validated against typed fields (date, email, int, bool) and invalid rows become named exceptions - not silent drops, not silent writes; A run cannot be marked complete while exceptions are unresolved, and every resolution (accept / fix / skip) requires a reason; Accept and fix count the row; skip drops it - the final row count is explainable to the last record

## What it does today

- Automated suite: 30/30 checks, all green.
...
2. In the left sidebar, click **XchangeHub**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
3. The top cards are the live numbers - **Exchanges** - read them left to right for the state of things.
4. The main sections are **Exchanges, Runs, Exception review, New exchange**. The records table's key columns: Name, Direction, Fields, Run, Exchange.
...
XchangeHub today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
