...
# UpgradeSentinel - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Upgrade governance: components with semver versions, four named compatibility checks per target version, plans that refuse sideways/downgrade targets, and execution gated on all-green checks plus a TESTED rollback backup. Rollback restores versions and is recorded.

_Freemius store product #38547._

## What it replaces

- [WP Rollback](https://wordpress.org/plugins/wp-rollback/)
- [UpdateGuard](https://wordpress.org/plugins/updateguard/)
- [PlugVersions](https://wordpress.org/plugins/plugversions/)
- [Version Hopper](https://wordpress.org/plugins/version-hopper/)

## The wedge (why ours wins)

Execution is GATED: all four compatibility checks (php-compat, db-migration, api-breaks, dependency-tree) must be green, a rollback backup must be attached, and the backup must have been TESTED - re-attaching a new backup invalidates the prior test; Semver math is real: 1.10.0 > 1.9.0 numerically, sideways/downgrade targets are refused at plan time; Rollback is a first-class recorded act: it restores the component version, and re-upgrading then needs a fresh plan against the restored version
...
2. In the left sidebar, click **UpgradeSentinel**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
3. The top of the page carries the live summary - read it left to right for the state of things.
4. The main sections are **Register component** - work top to bottom.
...
UpgradeSentinel today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
