...
# WebhookForge - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Signed webhook operations: HTTPS-only endpoints with per-endpoint secrets, event subscriptions, HMAC signatures with freshness windows, exponential backoff retries, and a permanent dead-letter trail.

_Freemius store product #38556._

## What it replaces

- [HookAnywhere](https://wordpress.org/plugins/hookanywhere/)
- [Webhook Manager](https://wordpress.org/plugins/webequipe-webhook-manager/)
- [FlowSystems Webhook Actions](https://wordpress.org/plugins/flowsystems-webhook-actions/)
- [Hookly](https://wordpress.org/plugins/hookly-webhook-automator/)

## The wedge (why ours wins)

Every delivery is HMAC-SHA256 signed over timestamp.payload with a 5-minute freshness window and hash_equals verification - receivers can actually trust what arrives; Failures retry on true exponential backoff (60s, 120s, 240s...) capped at an hour, and a fifth failure dead-letters instead of vanishing; Early retries are rejected: the schedule is the schedule

## What it does today

- Automated suite: 30 passed, 0 failed.
...
2. In the left sidebar, click **WebhookForge**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
3. The top cards are the live numbers - **Active endpoints, Delivered, Retrying, Dead-lettered** - read them left to right for the state of things.
4. The main sections are **Endpoints, Recent deliveries, Add endpoint, Dispatch test event**. The records table's key columns: ID, URL, Events, Status, Endpoint.
...
WebhookForge today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
