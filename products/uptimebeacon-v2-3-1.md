...
# UptimeBeacon - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Health-check engine: monitors with per-monitor fail/recover thresholds, checks with latency, incidents that auto-open on consecutive failures and auto-close on consecutive recoveries, windowed uptime %, p95 latency, and MTTR.

_Freemius store product #38548._

## What it replaces

- [Pingview](https://wordpress.org/plugins/pingview-external-uptime-monitoring/)
- [LMN Site Monitor](https://wordpress.org/plugins/lmn-site-monitor/)
- [Watchtower](https://wordpress.org/plugins/watchtower/)
- [Lukstack Uptime Monitor](https://wordpress.org/plugins/lukstack-uptime-monitor/)

## The wedge (why ours wins)

Flapping protection is structural: incidents open on N CONSECUTIVE failures and close on M consecutive successes - a blip never pages anyone; The math is local and exact: uptime % over any window, nearest-rank p95 latency, MTTR from closed incidents; Monitors have independent streaks - one flapping service never pollutes another's incident record

## What it does today

- Automated suite: 23 passed, 0 failed.
...
2. In the left sidebar, click **UptimeBeacon**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
3. The top of the page carries the live summary - read it left to right for the state of things.
4. The main sections are **Add monitor** - work top to bottom.
...
UptimeBeacon today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
