...
# CachePilot - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Cache policy per content type: validated TTLs (SWR bounded by max-age), correct Cache-Control synthesis (no-store at 0, private fallback for unconfigured scopes), purge-key computation on edits.

_Freemius store product #38339._
...
2. In the left sidebar, click **CachePilot**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
3. The top of the page carries the live summary - read it left to right for the state of things.
4. The main sections are **Set policy**. The records table's key columns: Scope, Header sent.
...
CachePilot today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
