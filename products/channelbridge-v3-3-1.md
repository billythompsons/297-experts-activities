...
# ChannelBridge - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Notification routing done right: triggers -> channels (email/slack/discord/webhook) with validated targets, {title} templates with unfilled-placeholder detection, quiet hours that wrap midnight, and per-route throttling.

_Freemius store product #38345._
...
2. In the left sidebar, click **ChannelBridge**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
3. The top of the page carries the live summary - read it left to right for the state of things.
4. The main sections are **Add route**. The records table's key columns: Trigger, Channel, Target, Template, Throttle.
...
ChannelBridge today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
