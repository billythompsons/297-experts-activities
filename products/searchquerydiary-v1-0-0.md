...
# Search Query Diary - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Site search analytics. The product is a WordPress plugin with a pure-PHP testable engine, a folk-styled admin report, and a bounded, evidence-first workflow. Fully local: nothing leaves the server.

## What it replaces

- [Lost Search Result Tracker – WordPress plugin](https://wordpress.org/plugins/dits-lost-search-query/)
- [yuyuwp/WP-Search-Logger](https://github.com/yuyuwp/WP-Search-Logger)
- [Fix Zero-Result Searches in WordPress – WP Search Insights](https://wpsi.io/docs/understanding-and-acting-on-zero-result-searches/)

## The wedge (why ours wins)

Zero-result terms ranked by frequency with concrete write/interlink suggestions - not just a query log; Zero-result rate KPI tracked as a first-class number; Bounded diary (500 newest) - the table never grows unbounded like naive loggers; Fully local privacy boundary: no external service, no account, no data leaving the server; Zero signup: works on activation; competitors gate basics behind accounts or upsells
...
2. In the left sidebar, click **Searchquerydiary**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
...
Search Query Diary today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
