...
# OrderBeacon - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Order status pipeline with legal transitions enforced, cancellation reasons required, SLA breach alerts, and a customer-facing notification trail.

_Freemius store product #38493._

## The wedge (why ours wins)

A real status machine: pending -> processing -> on_hold -> shipped -> delivered, with illegal transitions refused (no reopens, no skips) instead of free-form status edits; Accountability built in: cancellations require a reason, every transition is timestamped history; SLA breach detection out of the box: orders aging past 24/48/72h limits surface as breaches, oldest wait first; Customer-facing timeline separates what customers see from internal notes; All local: no carrier APIs, no SaaS, no keys
...
2. In the left sidebar, click **OrderBeacon**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
3. The top of the page carries the live summary - read it left to right for the state of things.
4. The main sections are **SLA breaches, New order, Notification trail**. The records table's key columns: Order, Customer, Status, Age, Limit.
...
OrderBeacon today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
