...
# TicketNest - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Support ticketing with enforced lifecycle and SLA math: priorities carry first-response and resolution deadlines, replies stamp first response exactly once, breaches are computed per ticket, reopen is single-use and demands a fresh reply before the next resolve.

_Freemius store product #38542._

## What it replaces

- [WP Ticket](https://wordpress.org/plugins/wp-ticket/)
- [SupportCandy](https://wordpress.org/plugins/supportcandy/)
- [ELEX Helpdesk](https://wordpress.org/plugins/elex-helpdesk-customer-support-ticket-system/)
- [Happy Helpdesk](https://wordpress.org/plugins/happy-helpdesk-support-ticket-system/)

## The wedge (why ours wins)

SLAs are real math: per-priority first-response AND resolution deadlines (urgent 1h/4h), breaches computed against the current time and surfaced; First response stamps once and never restamps - the metric cannot be gamed by a late flurry of replies; Lifecycle discipline: only pending tickets resolve, only resolved close or reopen, a ticket reopens exactly once and needs a fresh reply before it can resolve again
...
2. In the left sidebar, click **TicketNest**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
3. The top of the page carries the live summary - read it left to right for the state of things.
4. The main sections are **SLA breaches, Add agent / open ticket** - work top to bottom.
...
TicketNest today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
