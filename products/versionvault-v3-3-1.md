...
# VersionVault - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Update governance: a version inventory of plugins/themes/core, compatibility findings bound to exact version pairs, and an update pipeline that requires changelog + staging + backup evidence and a second approver before anything is applied.

_Freemius store product #38553._

## What it replaces

- [WP Rollback](https://wordpress.org/plugins/wp-rollback/)
- [PlugVersions](https://wordpress.org/plugins/plugversions/)
- [Version Control Your Content](https://wordpress.org/plugins/version-control-your-content/)
- [Simple Editor Control](https://wordpress.org/plugins/simple-editor-control/)

## The wedge (why ours wins)

Updates cannot apply without evidence: changelog read, staging tested, backup taken - and a second person approves. Self-approval is rejected; Semver comparison is real (numeric components, prerelease ordering), so "not newer than installed" is computed, not assumed; Compatibility findings bind to exact version pairs and clear when superseded - conflicts shown are conflicts NOW, not history

## What it does today

- Automated suite: 35/35 checks, all green.
...
2. In the left sidebar, click **VersionVault**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
3. The top cards are the live numbers - **Components, Proposed, Approved** - read them left to right for the state of things.
4. The main sections are **Inventory, Updates, Register component, Propose update**. The records table's key columns: Component, Kind, Version, Conflicts, ID.
...
VersionVault today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
