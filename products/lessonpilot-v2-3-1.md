...
# LessonPilot — - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Gated lesson progression and assessments with recorded attempts.

_Freemius store product #38461._

## The wedge (why ours wins)

LessonPilot is small and strict: progression is gated by the engine itself (an out-of-order attempt is refused, not just hidden in the UI), failed attempts are recorded but never complete a lesson, per-lesson pass thresholds and best scores are first-class data, and the certificate is computed from recorded evidence. The big LMS plugins optimize for selling courses; LessonPilot optimizes for proving learning happened
...
2. In the left sidebar, click **LessonPilot**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
...
LessonPilot — today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
