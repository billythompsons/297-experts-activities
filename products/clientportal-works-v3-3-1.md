...
# ClientPortal Works - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Per-client private spaces: allow-listed emails (validated, case-insensitive), shared items with http(s)-only URLs, access checks that deny strangers by default. Shortcode [clientportal_works space="slug"].

_Freemius store product #38348._
...
2. In the left sidebar, click **ClientPortal Works**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
3. The top of the page carries the live summary - read it left to right for the state of things.
4. The main sections are **New space**. The records table's key columns: Client, Slug, Allowed, Items, Share item.
...
ClientPortal Works today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
