...
# RedirectAtlas — - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Redirect maps with write-time loop detection, wildcard prefixes, chain reporting, and a forward-only migration hit log.

_Freemius store product #38509._

## The wedge (why ours wins)

Write-time loop/chain rejection, longest-prefix wildcards, explicit chain report, forward-only migration log, and a built-in path tester in the admin
...
2. In the left sidebar, click **Redirectatlas**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
...
RedirectAtlas — today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
