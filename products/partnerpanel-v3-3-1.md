...
# PartnerPanel - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Partner onboarding stages, service-level evidence, review scoring, and churn decisions with reasons - a real relationship panel for partner programs.

_Freemius store product #38489._

## The wedge (why ours wins)

Relationship operations, not referral accounting: onboarding stages enforced in order (agreement -> setup -> first_delivery -> review); Reviews are earned: a review requires evidence entries AND SLA records, and produces a weighted score (60% SLA compliance, 40% evidence depth); Consequences are automatic: active partners scoring under 0.5 drop to at_risk; Churn is a recorded decision with a mandatory reason, not a delete; All local: no affiliate network, no tracking pixels, no external calls
...
2. In the left sidebar, click **PartnerPanel**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
3. The top of the page carries the live summary - read it left to right for the state of things.
4. The main sections are **New partner** - work top to bottom.
...
PartnerPanel today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
