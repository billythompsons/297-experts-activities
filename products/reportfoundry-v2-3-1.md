...
# ReportFoundry — - 297 in 501: Experts Activities

![The five plans, one ladder](tier-ladder.png)

## What it is

Saved reports with a fixed column contract, validated runs, one-click CSV export, and numeric totals. Run history is forward-only; every row is validated against the column contract.

_Freemius store product #38512._

## The wedge (why ours wins)

Fixed column contract with per-row validation, RFC 4180 CSV, forward-only run history, honest numeric totals (no locale-string coercion), computed cadence digest
...
2. In the left sidebar, click **Reportfoundry**. The folk-styled admin page opens: paper background, eyebrow line, serif-italic headline.
...
ReportFoundry — today carries 0 of the 297 canonically. The vision is every product carrying all 297. For each feature this product does not yet ship: what to build, how, and what it needs (free tiers and open source first, per the standing rule).
